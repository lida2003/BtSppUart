...
#define SPP_TAG            "BtSppUart"
#define SPP_SERVER_NAME     SPP_TAG
#define EXAMPLE_DEVICE_NAME SPP_TAG


#define TTL_UART_NUM        (UART_NUM_1)
#define TTL_TEST_TXD        (GPIO_NUM_4)
#define TTL_TEST_RXD        (GPIO_NUM_5)
...
static uint32_t esp_ssp_handle = 0;
static bool bt_is_connected = false;
QueueHandle_t ttl_uart_queue = NULL;

void ttl_uart_task(void *pvParameters)
{
    uart_event_t event;
        ...
        if (xQueueReceive(ttl_uart_queue, (void * )&event, (TickType_t)portMAX_DELAY)) {
            ...
            case UART_DATA:
                if ((event.size)&&(bt_is_connected)) {
                    ...
                    if(temp == NULL){
                        ESP_LOGE(SPP_TAG, "%s malloc.1 failed\n", __func__);
                        break;
                    }
                    memset(temp,0x0,event.size);
                    uart_read_bytes(TTL_UART_NUM, temp, event.size, portMAX_DELAY);
					
					ESP_LOGI(SPP_TAG, "ttl read %d Bytes\n", event.size);
					if (event.size < 128) {
						esp_log_buffer_hex(SPP_TAG, temp, event.size);
					}
...
void ttl_uart_init(void)
{
    ...
    //Set UART pins
    uart_set_pin(TTL_UART_NUM, TTL_TEST_TXD, TTL_TEST_RXD, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
    xTaskCreate(ttl_uart_task, "ttl_uart_task", 2048, (void*)TTL_UART_NUM, 8, NULL);
}
